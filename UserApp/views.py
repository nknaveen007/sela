    ...
    if result['status']=='approved':
      ...
      try:
        ...
        if userserialiser.data['PhoneNumber']:
           return JsonResponse({'otp status':result,'user details':userserialiser.data},safe=False)
         ...
         if er=='UserData matching query does not exist.':
           userdata_serializer=UserSerialiser(data={'PhoneNumber':number[2:]})
           if userdata_serializer.is_valid():
             userdata_serializer.save()  
             user=UserData.objects.get(PhoneNumber=number[2:])
             userdata_serializer=UserSerialiser(user)
             return JsonResponse({'otp status':result,'user':'not registered','userdata':userdata_serializer.data},safe=False,status=200) 
           return JsonResponse('Failed to Add',safe=False)     
         return JsonResponse('something wrong',safe=False,status=500)    

    else:
       return JsonResponse('phone number or otp is not wrong!',safe=False)
  ...
  try:
    if request.method=='GET':
        ...
        if user.UserStatus==1 and user.Name=='':
          ...
          updatedData={'SelaId':selaId,'Name':users_data['Name'],'UserStatus':0}
          ...
          return JsonResponse('Failed to Update',safe=False)
           ...
           user_serializer=UserSerialiser(user,data={'Name':users_data['Name']}) 
           if user_serializer.is_valid():
            user_serializer.save()
